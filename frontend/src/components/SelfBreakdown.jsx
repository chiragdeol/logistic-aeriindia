...
export default function SelfBreakdown({ data, customerName }) {
  ...
  return (
    <div className="rounded-md bg-white border border-slate-200 shadow-sm overflow-hidden" data-testid="self-breakdown">
      ...
      <div className="px-5 sm:px-6 py-5 bg-slate-900 text-white">
        ...
        {data.chargeable_weight > 0 && (
          <div className="flex items-baseline justify-between pt-3 border-t border-slate-700/50 text-slate-300">
            ...
            <div className="font-mono tabular-nums text-sm text-yellow-400 font-semibold" data-testid="self-breakdown-per-kg">
      ...
      {data.notes?.length > 0 && (
        <div className="px-5 sm:px-6 py-3 space-y-1.5 bg-slate-50" data-testid="self-breakdown-notes">
          {data.notes.map((n, i) => (
