import { Plane, User, MapPin, Package as PkgIcon, Info } from "lucide-react";
...
export default function Breakdown({ data, customerName }) {
  ...
  return (
    <div className="rounded-md bg-white border border-slate-200 shadow-sm overflow-hidden" data-testid="rate-breakdown">
      {/* Header strip */}
      <div className="px-5 sm:px-6 py-4 bg-gradient-to-r from-yellow-400 to-yellow-300 flex items-center justify-between">
        <div className="flex items-center gap-2 text-slate-900">
          <Plane className="w-4 h-4" />
          <span className="font-semibold text-sm tracking-wider">DHL EXPRESS</span>
        </div>
        <span className="text-[10px] tracking-[0.25em] uppercase text-slate-900/70 font-medium">Quote</span>
      </div>

      {/* Customer + destination */}
      <div className="px-5 sm:px-6 py-4 border-b border-slate-100 space-y-2">
        {customerName && (
          <div className="flex items-center gap-2 text-sm text-slate-700" data-testid="breakdown-customer">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-display text-lg sm:text-xl text-slate-900 leading-tight" data-testid="breakdown-country">
              {data.country.name}
              <span className="text-slate-400 font-normal ml-2 text-sm">({data.country.code})</span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-slate-500 mt-0.5">
              <span className="font-mono">Zone {data.zone}</span>
              <span>·</span>
              <PkgIcon className="w-3 h-3" />
              <span>{data.shipment_type === "document" ? "Document" : "Non-Document"}</span>
              <span>·</span>
