import { User, MapPin, Package as PkgIcon, Info, ShieldCheck } from "lucide-react";
...
export default function UpsBreakdown({ data, customerName }) {
  ...
  return (
    <div className="rounded-md bg-white border border-slate-200 shadow-sm overflow-hidden" data-testid="ups-breakdown">
      {/* Header strip */}
      <div className="px-5 sm:px-6 py-4 bg-gradient-to-r from-amber-800 to-amber-600 flex items-center justify-between">
        <div className="flex items-center gap-2 text-white">
          <ShieldCheck className="w-4 h-4 text-yellow-400" />
          <span className="font-semibold text-sm tracking-wider text-white">UPS SAVER</span>
        </div>
      ...
      <div className="px-5 sm:px-6 py-4 border-b border-slate-100 space-y-2">
        {customerName && (
          <div className="flex items-center gap-2 text-sm text-slate-700" data-testid="ups-breakdown-customer">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-display text-lg sm:text-xl text-slate-900 leading-tight" data-testid="ups-breakdown-country">
              {data.country.name}
      ...
      {data.notes?.length > 0 && (
        <div className="px-5 sm:px-6 py-3 space-y-1.5 bg-slate-50" data-testid="ups-breakdown-notes">
          {data.notes.map((n, i) => (
